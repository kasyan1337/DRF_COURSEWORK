...
class HabitAPITests(APITestCase):
    def setUp(self):
        ...
        self.other_habit_detail_url = reverse(
            "habit-detail", kwargs={"pk": self.other_habit.pk}
        )
    ...
    def test_update_other_users_habit(self):
        ...
        other_habit_exists = Habit.objects.filter(
            pk=self.other_habit.pk, user=self.other_user
        ).exists()
